import { LOG_TYPE, backLog } from 'common/etc/log';
import { Param, settings } from 'commonBack';
import sqlite from 'sqlite3';
import { schemaList } from 'database/install/schema';
import { ConfigParam } from 'commonBack';
...
export function createTable(TableschemaQuery: string[]) {
  TableschemaQuery.forEach((query) => {
    database.run(query, (err: Error) => {
      if (err) {
        backLog(`executing query: ${err.message}`, LOG_TYPE.ERROR);
      }
    });
  });
}
